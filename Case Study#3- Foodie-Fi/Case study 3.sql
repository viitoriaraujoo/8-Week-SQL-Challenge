...
CREATE TABLE plans (
  plan_id INTEGER,
  plan_name VARCHAR(13),				-- 13 é o número máximo de caracteres 
  price DECIMAL(5,2)					-- número 5 indica o comprimento total do número, e o 2 indica quantos desses dígitos estão após o ponto decimal
);
...
--Qual é a distribuição mensal dos valores destart_date para o plano trial em nosso conjunto de dados?
...
--Quais valores de start_date do plano ocorrem após o ano de 2020 em nosso conjunto de dados? Mostre a distribuição por contagem de eventos para cada plan_name.
...
--Qual é a contagem de customer e a porcentagem de customerque abandonaram, arredondada para 1 casa decimal?
SELECT 
...
--Quantos clientes cancelaram logo após o período de teste gratuito inicial - qual é a porcentagem disso arredondada para o número inteiro mais próximo?
WITH cte_churn AS 			-- tabela temporária
(
SELECT *, 
...
--Qual é o número e a porcentagem de planos de clientes após o período inicial de teste gratuito?
...
planning AS ----- crição tabela temporária 
	(
	SELECT z.next_plan,
	       COUNT(DISTINCT customer_id) AS customer_count, -- total de clientes que utilizaram plano grátis
	       (100 * CAST(COUNT(DISTINCT customer_id) AS FLOAT) / (SELECT COUNT(DISTINCT customer_id) FROM subscriptions)) AS percentage --porcentagem de clientes distintos em relação ao número total de clientes
...
-- Qual é a contagem de clientes e a distribuição percentual de todos os valores de plan_name em 31 de dezembro de 2020?
...
-- Quantos dias, em média, um cliente leva para fazer upgrade para um plano anual a partir do dia em que se inscreve no Foodie-Fi?
WITH trial_plan AS (
    SELECT customer_id,
...
--Quantos clientes fizeram downgrade de um plano mensal Pro para um plano mensal Básico em 2020?
